...
import { getGoogleAuthUrl } from '../utils/authUrls';
...
const SignUp: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email.endsWith('@umt.edu.pk')) { setError('Only UMT email addresses (@umt.edu.pk) are allowed.'); return; }
  ...
  const handleGoogleSignUp = () => {
    if (!acceptedTerms) {
      setError('You must accept the Terms & Conditions before signing up with Google.');
      return;
    }
    setError(null);
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="bg-white border border-gray-100 rounded-3xl shadow-xl p-5 sm:p-8 md:p-10 relative overflow-hidden">
          ...
          <div className="text-center mb-7">
            ...
            <p className="text-gray-500 text-sm">Sign up as a patient to start your wellness journey</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  type="text" required disabled={isBusy}
            ...
            <div>
              ...
              <input
                type="date" required disabled={isBusy}
                max={new Date().toISOString().split('T')[0]}
                className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all text-sm text-gray-700 disabled:opacity-60"
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl text-white font-bold text-base shadow-lg transform transition-all mt-1 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none bg-blue-600 hover:bg-blue-700 shadow-blue-600/30"
            >
              {isSubmitting ? (
                <>
          ...
          <p className="mt-4 text-center text-xs text-gray-400">
            Doctor accounts are assigned by an administrator after signup.
          </p>
